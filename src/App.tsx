...
function App() {
  const [climateData, setClimateData] = useState<ClimateData>({} as unknown as ClimateData);
  ...
  useEffect(() => {
    ...
    showChart(
      climateData.precipData.monthVals,
      climateData.tempData.monthVals
    );
  }, [climateData]);

  const updateData = async () => {
    const newData = await getClimateData(1980, 1999, 'usa');
